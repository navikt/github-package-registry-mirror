...
const { localFileExists, streamToString, waitForStreamToEnd, modifiedHeadersWithAuth, parsePathAsArtifact, isNavPackage, isMavenMetadataXml } = require('./utils');
const { createGCSStorage } = require('./storage');

function createApp(deps) {
    const fetchFn = (deps && deps.fetchFn) || require('node-fetch');
    const storageFn = (deps && deps.storageFn) || createGCSStorage('github-package-registry-storage');
    const getTokenFn = (deps && deps.getTokenFn) || async function(tokenName) {
        if (await localFileExists(tokenName)) {
            return fs.readFileSync(tokenName, 'utf-8').trim();
        } else {
            const stream = await storageFn.file('credentials/' + tokenName).createReadStream();
    ...
    async function existsInCache(fileName) {
        ...
        try {
            ...
            let file = storageFn.file(fileName);
    ...
    async function handleSimple(req, res, repo, path) {
        try {
            const token = await getTokenFn('github-token');
            ...
            const response = await fetchFn(resolvedGithubPath, {
        ...
        try {
            ...
            if (!exists) {
                ...
                if (response.status === 301 || response.status === 302) {
                    ...
                    const artifactResponse = await fetchFn(location);
                    ...
                    const writeStream = await storageFn.file(file).createWriteStream();
            ...
            const readStream = await storageFn.file(file).createReadStream();
    ...
    app.get('/dummy', async (_req, res) => {
        try {
            ...
            const secret = await getTokenFn('dummy-token');
